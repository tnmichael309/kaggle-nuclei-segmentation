...
def make_fpn_c0(input_image, out_planes, is_pooling=False):
    x = KL.ZeroPadding2D((3, 3))(input_image)
    x = KL.Conv2D(out_planes, (7, 7), strides=(2, 2), name='conv1', use_bias=True)(x)
...
def make_fpc_layer_c(input_tensor, kernel_size, filters, stage, block_count, strides=(2, 2), use_bias=True):
    x = conv_block(input_tensor, kernel_size, filters, stage=stage, block='a', strides=strides, use_bias=use_bias)
    for i in range(block_count):
        x = identity_block(x, kernel_size, filters, stage=stage, block=chr(98 + i))
        
    return x
    
def resnet_graph(input_image, stage5=True, block_counts=[2,3,5,2]):
    assert (stage5 is True and len(block_counts) == 4) or (stage5 is False and len(block_counts) == 3)
    
    # Stage 1
    C1 = x = make_fpn_c0(input_image, 60, is_pooling=False)
    
    # Stage 2
    C2 = x = make_fpc_layer_c(x, 3, [64, 64, 256], stage=2, block_count=block_counts[0], strides=(1, 1))
    
    # Stage 3
    C3 = x = make_fpc_layer_c(x, 3, [128, 128, 512], stage=3, block_count=block_counts[1])
    
    # Stage 4
    C4 = x = make_fpc_layer_c(x, 3, [256, 256, 1024], stage=4, block_count=block_counts[2])
    
    if stage5:
        C5 = x = make_fpc_layer_c(x, 3, [512, 512, 2048], stage=5, block_count=block_counts[3])
    else:
        C5 = None
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # resnet_graph(input_image, stage5=True, block_counts=[2,3,5,2])
        _, C2, C3, C4, C5 = resnet_graph(input_image, stage5=True, block_counts=[3,4,6,3])
